...
$flow = Join-Path $RepoRoot "flows\idcevo\testcases\$CaseId\idcevo.yaml"
...
# Resolve Maestro command robustly
$maestroCmd = $null
...
$dltOut = Join-Path $runRoot "dlt\idcevo_capture.dlt"
...
& node $dltJs start $DltIp $DltPort $dltOut "IDCEVO"
if ($LASTEXITCODE -ne 0) { throw "Failed to start DLT capture" }

$maestroExit = 0
try {
    Write-Host "Running Maestro flow on IDCEvo device..."
    ...
    & node $dltJs stop "0" "0" "0" "IDCEVO" | Out-Host
}
...
# Try to pull video evidence from latest Maestro tests run if available
$videoCount = 0
