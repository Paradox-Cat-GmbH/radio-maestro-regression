...
set "OUT_DIR=%ART_DIR%\maestro_demo"
if not exist "%OUT_DIR%" mkdir "%OUT_DIR%"

if "%USE_DEVICE_FLAG%"=="1" (
  "%MAESTRO_EXE%" --device "%DEVICE_ID%" test flows\demo --test-output-dir "%OUT_DIR%"
) else (
  "%MAESTRO_EXE%" test flows\demo --test-output-dir "%OUT_DIR%"
)
