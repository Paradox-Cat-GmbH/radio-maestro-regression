...
set "BASE_OUT=%ART_DIR%\maestro_lightning"
if not exist "%BASE_OUT%" mkdir "%BASE_OUT%"

set FLOW1=flows\demo\IDCEVODEV-478199__all_stations_select.yaml
set FLOW2=flows\demo\IDCEVODEV-478229__search_select_station.yaml
set FLOW3=flows\demo\IDCEVODEV-478202__miniplayer_skip_next.yaml
set FLOW4=flows\demo\IDCEVODEV-478210__swag_skip_next.yaml
set FLOW5=flows\demo\IDCEVODEV-478205__bim_skip_next.yaml

for %%F in ("%FLOW1%" "%FLOW2%" "%FLOW3%" "%FLOW4%" "%FLOW5%") do (
  set "FNAME=%%~nF"
  set "OUT=%BASE_OUT%\!FNAME!"
  if not exist "!OUT!" mkdir "!OUT!"
  echo [RUN] %%~nxF
  if "%USE_DEVICE_FLAG%"=="1" (
    "%MAESTRO_EXE%" --device "%DEVICE_ID%" test %%~fF --test-output-dir "!OUT!"
  ) else (
    "%MAESTRO_EXE%" test %%~fF --test-output-dir "!OUT!"
  )
  if errorlevel 1 exit /b %ERRORLEVEL%
)
